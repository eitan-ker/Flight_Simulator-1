...
int setToSimulatorCommand::execute(vector<string> &str, int i) {
    Singleton *t = t->getInstance();
    string simpath = str.at(i + 2).substr(1, str.at(i + 2).size() - 2);
    Var_Data *temp = new Var_Data{t->getAllVarsFromXMLMMap()[simpath].get_value(),
                                  simpath};//create new float,sim object in the heap
  ...
  from now on we can access the variable using this name*/
    t->getsymbolTableToServerMap().emplace(str.at(i - 1),
                                           temp);//insert the new name,(float,sim) object pair to symbolTableToServer map
    return 3;
}
